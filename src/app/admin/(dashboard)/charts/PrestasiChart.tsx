...
export default function PrestasiChart({ data, isDrilling, drillFakultas }: Props) {
  const title = isDrilling
    ? `Status Prestasi Prodi — ${drillFakultas}`
    : 'Status Prestasi Lulusan';
  
  const pieData = [
    { name: 'Memiliki Prestasi', value: data.sudah, fill: '#8b5cf6' }, // violet-500
    { name: 'Tidak Ada Prestasi', value: data.belum, fill: '#94a3b8' }, // slate-400
  ].filter(d => d.value > 0);
    ...
    <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl overflow-hidden">
      <div className="px-5 py-3.5 border-b border-[var(--color-border)] flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Star size={15} className="text-[var(--color-text-subtle)]" />
