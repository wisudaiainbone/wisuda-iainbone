...
export default function SurveiChart({ data, isDrilling, drillFakultas }: Props) {
  const title = isDrilling
    ? `Kepatuhan Survei Prodi — ${drillFakultas}`
    : 'Kepatuhan Pengisian Survei';
  
  const pieData = [
    { name: 'Sudah Mengisi', value: data.sudah, fill: '#14b8a6' }, // teal-500
    { name: 'Belum Mengisi', value: data.belum, fill: '#ef4444' }, // red-500
  ].filter(d => d.value > 0);
    ...
    <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl overflow-hidden">
      <div className="px-5 py-3.5 border-b border-[var(--color-border)] flex items-center justify-between">
        <div className="flex items-center gap-2">
          <ClipboardCheck size={15} className="text-[var(--color-text-subtle)]" />
