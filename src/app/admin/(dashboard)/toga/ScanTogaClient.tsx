...
import { warmUpTogaCache } from "@/actions/scanCache";
import { getRecentScans, getAllTogaScans } from "@/actions/scanHistory";
import { Html5Qrcode } from "html5-qrcode";
import { Camera, CameraOff, CheckCircle2, XCircle, AlertCircle, RefreshCw, Shirt, Search, Database, ClipboardList, X, FileDown } from "lucide-react";
...
export default function ScanTogaClient({ initialMeta }: { initialMeta: any }) {
  ...
  const handleWarmUp = async () => {
    setIsWarmingUp(true);
    const res = await warmUpTogaCache();
    if (res.success) {
  ...
  const handleExportXlsx = async () => {
    setIsExporting(true);
    try {
      const res = await getAllTogaScans();
      if (!res.success || !res.data) {
        alert('Gagal mengambil data scan toga.');
        return;
      }
      const rows = res.data as any[];

      const togaRows = [
        ['No', 'NIM', 'Nama', 'Fakultas', 'Prodi', 'Ukuran Toga', 'Waktu Ambil'],
        ...rows.map((r, i) => [
          i + 1,
          r.nim,
          r.nama_mahasiswa,
          ...
          r.prodi || '-',
          r.toga || '-',
          r.waktu_toga ? new Date(r.waktu_toga).toLocaleString('id-ID') : '-',
        ]),
      ];
      ...
      a.href = url;
      a.download = `Scan_Toga_Wisuda_${now}.xlsx`;
      a.click();
  ...
  useEffect(() => {
    ...
    const startScanner = async () => {
      ...
      try {
        html5QrCode = new Html5Qrcode('qr-reader-toga');
  ...
  const processScanByNim = async (nim: string) => {
    ...
    setScanResult(null);
    try {
      const res = await fetch('/api/scan/toga', {
        method: 'POST',
      ...
      if (data.success) {
        ...
        const isWarning = data.error === 'already_taken' || data.error === 'toga_not_filled';
  ...
  const processScan = async (decodedText: string) => {
    if (isProcessingRef.current) return;
    // Validasi format QR toga: harus mengandung underscore (format periodeSlug_tahun_urutan_nim)
    // dan bukan URL
    const isValidTogaQR = !decodedText.startsWith('http') && decodedText.includes('_') && decodedText.split('_').length >= 4;
    if (!isValidTogaQR) {
      speakResult('error');
    ...
    try {
      const res = await fetch('/api/scan/toga', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id_wisuda: decodedText }),
      });
      const data = await res.json();
      if (data.success) {
        speakResult('success');
        ...
        {!isCameraActive ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-6 bg-[var(--color-surface)]">
            ...
            <div className="text-center">
              <p className="text-lg font-bold text-[var(--color-text)] mb-1">Scan Toga</p>
          ...
          <>
            ...
            <style dangerouslySetInnerHTML={{
              __html: `
              #qr-reader-toga { width: 100% !important; border: none !important; }
              #qr-reader-toga__dashboard_section_swaplink { display: none !important; }
              #qr-reader-toga__dashboard_section_fsr { display: none !important; }
              #qr-reader-toga__status_span { display: none !important; }
              #qr-reader-toga__header_message { display: none !important; }
            `}} />

            {/* QR Reader — centered */}
            <div id="qr-reader-toga" className="w-full" />
          ...
          <div className="flex flex-col items-center gap-3">
            <div className="flex items-center justify-center gap-3">
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 px-5 py-3.5 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white rounded-2xl font-bold shadow-xl text-sm transition-all active:scale-95 disabled:opacity-70 flex-1"
              >
                <Database size={16} />
                  ...
                  <div className="w-full bg-white/70 dark:bg-black/20 rounded-2xl p-4 border border-white/50 dark:border-white/10 text-left">
                    ...
                    {scanResult.data.toga && (
                      <div className="mt-3 p-4 rounded-2xl flex flex-col items-center text-center bg-emerald-100/80 border-2 border-emerald-300 dark:bg-emerald-800/80 dark:border-emerald-600">
                        <span className="text-xs font-bold text-[var(--color-text-muted)] flex items-center gap-1 mb-1">
                          <Shirt size={14} /> UKURAN TOGA
                        </span>
                        <span className="font-black text-6xl text-[var(--color-text)] leading-none">
                          {scanResult.data.toga}
                        </span>
                      </div>
                    )}
            ...
            <tbody className="divide-y divide-[var(--color-border)]">
              {recentScans.length > 0 ? (
                recentScans.map((s, idx) => {
                  const waktu = s.waktu_toga ? new Date(s.waktu_toga).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', second: '2-digit' }) : '-';
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden p-0 scrollbar-thin">
              <table className="w-full text-left border-collapse text-xs sm:text-sm">
                ...
                <tbody className="divide-y divide-[var(--color-border)]">
                  {recentScans.length > 0 ? (
                    recentScans.map((s, idx) => {
                      const waktu = s.waktu_toga ? new Date(s.waktu_toga).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) : '-';
                      const prodiStr = `${getFakultasSingkat(s.fakultas)}-${s.prodi_singkat || ''}`;
                      return (
                        <tr key={s.nim || idx} className="hover:bg-[var(--color-bg-secondary)]/60 transition-colors">
