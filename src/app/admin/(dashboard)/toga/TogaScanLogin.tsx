...
import { verifyTogaScanPassword } from "@/actions/togaScanAuth";
import { Lock, ArrowLeft, Shirt } from "lucide-react";
import { ThemeToggle } from "@/components/ui/ThemeToggle";
import Image from "next/image";

export default function TogaScanLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await verifyTogaScanPassword(password);
      if (res.success) {
        window.location.href = "/admin/toga?tab=scan";
      } else {
  ...
  return (
    <div className="h-screen overflow-hidden flex flex-col relative bg-[var(--color-bg)]">
      ...
      />
      <div className="absolute top-20 left-[10%] w-72 h-72 rounded-full bg-amber-800/20 blur-[100px] pointer-events-none" />
      <div className="absolute bottom-20 right-[10%] w-80 h-80 rounded-full bg-amber-500/10 blur-[100px] pointer-events-none" />
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl text-[var(--color-text)] focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition-all outline-none"
                  placeholder="Masukkan Password Scan Toga"
                  required
                />
              </div>
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full mt-2 group relative flex items-center justify-center gap-2 px-8 py-3.5 rounded-xl text-white text-sm font-bold transition-all duration-300 bg-amber-600 hover:bg-amber-700 overflow-hidden disabled:opacity-70 disabled:cursor-not-allowed"
              >
