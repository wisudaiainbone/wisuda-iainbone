...
export default function AdminPengaturanPage() {
  ...
  const [allowTogaScanLogin, setAllowTogaScanLogin] = useState(true);
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setAllowTogaScanLogin(getVal('allow_toga_scan_login', 'true') === 'true');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const resTogaScan = await updateSetting('allow_toga_scan_login', allowTogaScanLogin.toString());
        ...
        {activeTab === 'general' && (
          <form onSubmit={handleSave} className="w-full space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300 pb-24 sm:pb-0">
            <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl overflow-hidden flex flex-col">
              ...
              {/* Izinkan Akses Scan Toga */}
              <label className="px-6 py-4 flex flex-col items-start sm:flex-row sm:items-center justify-between gap-4 sm:gap-6 cursor-pointer hover:bg-[var(--color-bg-secondary)]/50 transition-colors border-t border-[var(--color-border)]">
                <div className="flex-1">
                  <h2 className="text-sm font-bold text-[var(--color-text)]">
                    Buka Akses Admin Scan Toga (Tanpa Login)
                  </h2>
                  {descriptions['allow_toga_scan_login'] ? (
                    <p className="text-xs text-[var(--color-text-subtle)] mt-1.5 leading-relaxed">
                      {descriptions['allow_toga_scan_login']}
                    </p>
                  ) : (
                    <p className="text-xs text-[var(--color-text-subtle)] mt-1.5 leading-relaxed">
                      Izinkan Panitia Toga mengakses rute /admin/toga (mode scan) dengan menggunakan Password Default Calon Wisudawan.
                    </p>
                  )}
                </div>
                <div className="flex items-center justify-start shrink-0">
                  <input
                    type="checkbox"
                    className="sr-only"
                    checked={allowTogaScanLogin}
                    onChange={(e) => setAllowTogaScanLogin(e.target.checked)}
                  />
                  <div className={`relative w-12 h-6 rounded-full transition-colors ${allowTogaScanLogin ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-700'}`}>
                    <div className={`absolute top-1 left-1 bg-white w-4 h-4 rounded-full transition-transform ${allowTogaScanLogin ? 'translate-x-6' : 'translate-x-0'}`}></div>
                  </div>
                </div>
              </label>
