              ...
              return (
                ...
                >
                  {periode.status === 'Sedang Dibuka' && (
                    <div className="absolute top-4 right-4 px-2.5 py-1 bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400 text-xs uppercase font-bold tracking-wider rounded-full flex items-center gap-1.5">
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                      Aktif
                    </div>
                  )}

                  <h3 className="text-lg font-bold text-[var(--color-text)] pr-16">{periode.nama_periode}</h3>
                  ...
                  <div className="mt-4 pt-4 border-t border-[var(--color-border)]">
                    <Link
                      ...
                      className={`flex items-center justify-center gap-2 w-full py-2.5 rounded-xl text-sm font-semibold transition-colors ${periode.status === 'Sedang Dibuka'
                    ...
                    return (
                      <tr key={periode.id} className="hover:bg-[var(--color-bg-secondary)]/50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-right">
                          <Link
                            href={`/admin/periode/${periode.id}`}
                            className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-600 dark:bg-emerald-900/20 dark:hover:bg-emerald-900/40 dark:text-emerald-400 font-medium transition-colors"
                          >
                            <Edit3 size={14} />
                            Edit
                          </Link>
