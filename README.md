...
- **Feature Toggles Real-time**: Admin dapat mengaktifkan/nonaktifkan fitur (edit profil, tampilkan toga/undangan, izinkan perbaikan, dll.) yang langsung aktif di halaman wisudawan tanpa cache delay. Dilengkapi dengan **Manajemen Akun Uji Coba (Dummy)** yang memungkinkan admin membuat akun simulasi instan (`DUMMY999`) untuk mencoba langsung UI/UX dari sisi mahasiswa dengan aman tanpa masuk hitungan statistik.
- **Manajemen Wisudawan Lanjutan**: Halaman tabel interaktif dengan toolbar terpadu satu baris (pencarian + tombol aksi bergaya *Tag Cloud*), filter Fakultas/Prodi, import massal (beserta template cerdas, validasi *Fuzzy-Match* otomatis dan *Strict Date Parsing* — wajib format YYYY-MM-DD secara teks untuk menghindari *timezone shifting*), export Excel, **hapus data satuan maupun massal** (otomatis bersihkan foto di Drive dan cache Redis), edit data lengkap, dan reset password.
...
- **Incremental Static Regeneration (ISR)**: Halaman publik utama (/) di-cache secara statis di Vercel Edge Network dan otomatis di-_revalidate_ di latar belakang setiap 5 menit (300 detik). Ini menghasilkan performa instan untuk ribuan pengunjung tanpa membebani _database_ Supabase atau Upstash Redis, namun data metrik (seperti jumlah pendaftar dan sisa kuota) tetap dapat terbarukan secara berkala.
